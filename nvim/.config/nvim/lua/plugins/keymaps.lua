return {
  {
    ...
    opts = function(_, opts)
      ...
      maps.n["<leader>tt"] = { function() require("neotest").run.run(vim.fn.expand "%") end, desc = "Run File" }
      maps.n["<leader>tF"] = { function() require("neotest").run.run(vim.uv.cwd()) end, desc = "Run All Test Files" }
      maps.n["<leader>tr"] = { function() require("neotest").run.run() end, desc = "Run Nearest" }
      maps.n["<leader>tg"] = { function() require("neotest").run.run_last() end, desc = "Run Last" }
      maps.n["<leader>ts"] = { function() require("neotest").summary.toggle() end, desc = "Toggle Summary" }
      maps.n["<leader>to"] = {
        function() require("neotest").output.open { enter = true, auto_close = true } end,
        desc = "Show Output",
      }
      maps.n["<leader>tO"] = { function() require("neotest").output_panel.toggle() end, desc = "Toggle Output Panel" }
      maps.n["<leader>tS"] = { function() require("neotest").run.stop() end, desc = "Stop" }
      maps.n["<leader>tw"] =
        { function() require("neotest").watch.toggle(vim.fn.expand "%") end, desc = "Toggle Watch" }
      maps.n["<leader>td"] = {
        function() require("neotest").run.run { strategy = "dap" } end,
        desc = "Debug Test",
      }
      maps.n["<leader>tb"] = { function() require("dap").toggle_breakpoint() end, desc = "Toggle Breakpoint" }
      maps.n["<leader>tc"] = { function() require("dap").continue() end, desc = "Debug Continue" }
      maps.n["<leader>tj"] = { function() require("dap").step_over() end, desc = "Debug Continue" }
      maps.n["<leader>ti"] = { function() require("dap").step_into() end, desc = "Debug Continue" }
                  ...
                  for _, symbol in ipairs(symbols) do
                    ...
                    if vim.fn.line "." - 1 >= start_line and vim.fn.line "." - 1 <= end_line then
                      ...
                      if symbol.children then
                        ...
                        if child_function then return child_function end
                      end
                    end
                  end
                end
      ...
      local ng = require "ng"
      maps.n["<leader>at"] = { function() ng.goto_template_for_component() end, silent = true, desc = "Go to template" }
      maps.n["<leader>ac"] =
        { function() ng.goto_component_with_template_file() end, silent = true, desc = "Go to component" }
      maps.n["<leader>aT"] = { function() ng.get_template_tcb() end, silent = true, desc = "Go to usages in templates" }

      maps.n["gd"] = { function() vim.lsp.buf.definition() end, desc = "Go to Definition" }
      maps.n["gr"] = { function() vim.lsp.buf.references() end, desc = "Find References" }
      ...
      -- Spectre mappings
      maps.n["<leader>R"] = { function() require("spectre").open() end, desc = "Open Spectre for search and replace" }
      maps.n["<leader>rw"] =
        { function() require("spectre").open_visual { select_word = true } end, desc = "Search current word" }
      maps.v["<leader>rs"] = { function() require("spectre").open_visual() end, desc = "Search visually selected text" }
          ...
          ["<leader>fu"] = {
            function()
              ...
              require("telescope.builtin").lsp_references {
                initial_mode = "normal",
                include_declaration = false, -- Reduce duplicates
                show_line = true, -- Provide line preview in results
                attach_mappings = function(_, map)
                  -- Automatically highlight the word in the main buffer
                  vim.cmd("match Search /\\<" .. word .. "\\>/") -- Highlight word under cursor
                  map("i", "<C-l>", function(prompt_bufnr)
                    vim.cmd "match none" -- Clear highlight on key press
